...
3. Install with `pip install -r requirements.txt`; set credentials in `.env` using
   `.env.example`. The shared `runtime.connection.connect()` is reused.
4. Copy `experiments/skill_learning/station.example.json` to `experiments/skill_learning/station.json` if it does not exist. Resource names
   match the queried machine: `arm`, `gripper`, `cam`, and `builtin`.

```sh
python -m experiments.skill_learning.trial inspect
```

Inspect only reads. It saves inventory and camera samples under
`experiments/skill_learning/runs/inspect-*`.
...
```sh
python -m experiments.skill_learning.trial teach pregrasp    # empty gripper, just above the source
python -m experiments.skill_learning.trial teach grasp      # fingers around source, object supported
python -m experiments.skill_learning.trial teach lift       # a small initial test lift, same orientation
python -m experiments.skill_learning.trial teach pour_ready # source upright above receiving container
python -m experiments.skill_learning.trial teach pour_tilt  # calibrated pour pose, accounting for lip position
```

Only the first three are needed for pickup trials. Set measured `workspace_mm`
bounds in experiments/skill_learning/station.json, review the poses, then set `calibrated` to true. Re-teaching
...
Edit `experiments/skill_learning/profiles/cup.json`: its 100 g value is an example, **replace with measured
...
```sh
python -m experiments.skill_learning.trial run --task pick                  # offline config validation; no connection
python -m experiments.skill_learning.trial run --task pick --execute        # one physical trial
python -m experiments.skill_learning.trial run --task pick --execute --repeat 5
python -m experiments.skill_learning.trial run --task pour --execute        # after reliable pickup
python -m experiments.skill_learning.trial label experiments/skill_learning/runs/TRIAL_ID success --poured-g 25 --notes 'no visible deformation'
python -m experiments.skill_learning.trial report
```
...
Each `experiments/skill_learning/runs/<trial>/` contains frozen profile/config/code JSON, `events.jsonl`,
...
loaded gripper. Inspect and manually recover before restarting. `python -m experiments.skill_learning.trial stop`
...
they are not a physics simulation. The older drink-menu files are isolated in `experiments/drink_menu/`.
Run these commands from the repository root. This harness is independent of the demo orchestrator.
