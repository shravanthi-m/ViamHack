...
function setup({recognition = true, synthesis = true, recording = false} = {}) {
  ...
  const state = {vision_configured: recording, demo: {enabled: false, active: false}, run: {mode: 'unattached', status: 'idle', events: []}};
  const context = vm.createContext({
    document: {getElementById: element, querySelector: element, querySelectorAll: () => [], createElement: element},
  ...
  return {element, posts, spoken, recognizer, windowEvents, context, cancelled: () => cancelled, stoppedTracks: () => stoppedTracks, recorder: () => mediaRecorder};
}
